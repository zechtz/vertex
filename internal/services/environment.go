...
// DefaultEnvironmentVariables returns empty environment variables for generic use
func (e *EnvironmentSetup) DefaultEnvironmentVariables() map[string]string {
	return map[string]string{
		// No default environment variables - users configure their own
	}
}
...
// SetupEnvironment sets up the environment variables
func (e *EnvironmentSetup) SetupEnvironment() *SetupEnvironmentResult {
	...
	log.Printf("[INFO] Setting up Service Manager environment...")
...
func (e *EnvironmentSetup) createCommonEnvFile(vars map[string]string) error {
	...
	_, err = file.WriteString("# Common Environment Variables for Microservices\n")
	if err != nil {
		return err
	}
	_, err = file.WriteString(fmt.Sprintf("# Generated by Service Manager on %s\n\n", time.Now().Format("2006-01-02 15:04:05")))
	if err != nil {
		return err
	}
	...
	// Write footer
	_, err = file.WriteString("echo \"✅ Common environment variables loaded successfully\"\n")
	if err != nil {
		return err
	}
...
func (e *EnvironmentSetup) updateShellProfile(vars map[string]string) error {
	...
	_, err = file.WriteString("\n# Service Manager Environment Variables\n")
	if err != nil {
		return err
	}
