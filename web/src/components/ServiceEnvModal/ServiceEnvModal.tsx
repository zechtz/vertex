...
import { X, Plus, Trash2, Save, RefreshCw, FileText, Upload } from "lucide-react";
...
export function ServiceEnvModal({ isOpen, onClose, serviceName }: ServiceEnvModalProps) {
  ...
  const fetchEnvVars = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/services/${serviceName}/env-vars`);
      if (!response.ok) {
        throw new Error(`Failed to fetch environment variables: ${response.status} ${response.statusText}`);
      }
      const data = await response.json();
      // Convert object to array format
      const vars = Object.entries(data.envVars || {}).map(([key, envVar]: [string, any]) => ({
        name: key,
        value: envVar.value || "",
  ...
  const handleSave = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to save environment variables: ${response.status} ${response.statusText}`);
      }
  ...
  const addEnvVar = () => {
    setEnvVars([...envVars, { name: "", value: "", description: "", isRequired: false }]);
  };
  ...
  const updateEnvVar = (index: number, field: keyof ServiceEnvVar, value: string | boolean) => {
  ...
  const handleBulkImport = () => {
    if (!bulkImportText.trim()) {
      addToast(toast.warning("No data to import", "Please enter environment variables to import"));
      return;
    }
    ...
    try {
      ...
      if (newVars.length === 0) {
        addToast(toast.error("No valid variables found", "Please check the format and try again"));
        return;
      }
      ...
      newVars.forEach(newVar => {
        ...
        if (existingIndex >= 0) {
          ...
          mergedVars[existingIndex] = { ...mergedVars[existingIndex], ...newVar };
        } else {
          // Add new variable
  ...
  const filteredEnvVars = envVars.filter(envVar => 
    ...
    (envVar.description && envVar.description.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <ErrorBoundarySection title="Environment Variables Error" description="Failed to load the environment variables form.">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold">{serviceName} - Environment Variables</h2>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Manage environment variables specific to the {serviceName} service
              </p>
            </div>
            ...
            <div className="mb-4 p-4 border rounded-lg bg-muted/50">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label htmlFor="bulk-import">Bulk Import Environment Variables</Label>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <p className="text-sm text-muted-foreground">
                  Paste semicolon-separated environment variables (e.g., VAR1=value1;VAR2=value2;VAR3=value3)
                </p>
                <Textarea
                  id="bulk-import"
                  placeholder="ACTIVE_PROFILE=dev;APM_ENABLED=false;CONFIG_PASSWORD=1kzwjz2nzegt3nest@ppra.go.tza1q@BmM0Oo;CONFIG_SERVER=nest-config-server;CONFIG_USERNAME=nest;cors.allowed.methods=*;cors.allowed.origins=*;SERVICE_PORT=8802"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center space-x-2 mt-6">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => updateEnvVar(actualIndex, "isRequired", checked)}
                        />
                        <Label htmlFor={`required-${actualIndex}`}>Required</Label>
                      </div>
                    </div>
                  </div>
                );
