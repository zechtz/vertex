...
import { X, Play, Trash2, Plus, ArrowUp, ArrowDown, GripVertical, Edit } from "lucide-react";
...
export function ConfigurationManager({
  ...
  const [configServices, setConfigServices] = useState<Array<{name: string; order: number; enabled: boolean}>>([]);
  ...
  const startCreating = () => {
    // Initialize with all services enabled in their current order
    const servicesWithConfig = services.map(service => ({
      name: service.name,
      order: service.order,
      enabled: true
  ...
  const startEditing = (config: Configuration) => {
    ...
    const missingServices = services
      .filter(service => !existingServices.find(es => es.name === service.name))
      .map(service => ({
        name: service.name,
        order: service.order,
        enabled: false
    ...
    const allServices = [...existingServices, ...missingServices]
      .sort((a, b) => a.order - b.order);
  ...
  const handleSaveConfig = async () => {
    if (!newConfigName.trim()) {
      addToast(toast.warning("Configuration name required", "Please enter a name for the configuration"));
      return;
    }
    ...
    if (enabledServices.length === 0) {
      addToast(toast.warning("No services selected", "Please select at least one service for the configuration"));
      return;
    }
  ...
  const handleApplyConfig = async (config: Configuration) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to apply configuration: ${response.status} ${response.statusText}`);
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <ErrorBoundarySection title="Configuration Manager Error" description="Failed to load the configuration manager.">
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <p className="text-muted-foreground">
              Manage service configurations and switch between different setups
            </p>
            <Button
              variant="outline"
              size="sm"
          ...
          {isCreating && (
            <div className="mb-6 p-6 border rounded-lg bg-blue-50 dark:bg-blue-900/20">
              <div className="space-y-4">
                ...
                <div>
                  <Label className="text-base font-medium">Services Configuration</Label>
                                  ...
                                  >
                                    ...
                                    <div className="flex gap-1">
                                      ...
                                      <Button
                                        ...
                                        disabled={index === configServices.length - 1}
                                      >
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button size="sm" onClick={handleSaveConfig} disabled={isSaving}>
                    <ButtonSpinner isLoading={isSaving} loadingText={editingConfig ? "Updating..." : "Creating..."}>
