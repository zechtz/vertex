...
export function ServiceFilesModal({
  ...
  const fetchServiceFiles = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/services/${serviceId}/files`);
      ...
      if (!response.ok) {
        ...
        const errorMessage =
          data.error ||
          `Failed to fetch service files: ${response.status} ${response.statusText}`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/services/${serviceId}/files/${encodeURIComponent(selectedFile.name)}`,
        {
          method: "PUT",
          headers: {
      ...
      setIsEditing(false);
      addToast(
        toast.success("File saved", `Successfully saved ${selectedFile.name}`),
      );
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
        <ErrorBoundarySection
          title="Service Files Error"
          description="Failed to load the service files editor."
        >
