...
export function GlobalConfigModal({
  ...
  const fetchGlobalConfig = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch global configuration: ${response.status} ${response.statusText}`);
      }
  ...
  const handleSave = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to save global configuration: ${response.status} ${response.statusText}`);
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <ErrorBoundarySection title="Global Configuration Error" description="Failed to load the global configuration form.">
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            <h2 className="text-xl font-semibold">Global Application Configuration</h2>
            <p className="text-sm text-muted-foreground">
              Configure global settings for the NeST Service Manager
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="projectsDir" className="text-base font-medium">
                  Projects Directory
                </Label>
                <p className="text-sm text-muted-foreground">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig(prev => ({ ...prev, projectsDir: e.target.value }))
                    }
                ...
                <div className="text-xs text-muted-foreground">
                  Current: <code className="bg-muted px-1 rounded">{config.projectsDir || 'Not set'}</code>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Custom Java installation path (optional). Leave empty to use system default.
                </p>
                <div className="flex gap-2">
                  <Input
                    id="javaHome"
                    ...
                    onChange={(e) =>
                      setConfig(prev => ({ ...prev, javaHomeOverride: e.target.value }))
                    }
                ...
                <div className="text-xs text-muted-foreground">
                  Current: <code className="bg-muted px-1 rounded">{config.javaHomeOverride || 'System default'}</code>
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-orange-700 dark:text-orange-300 mt-1">
                        Changing these settings may affect how services are started and managed. 
                        Consider restarting running services after saving changes.
                      </p>
                    </div>
                  </div>
                </div>
