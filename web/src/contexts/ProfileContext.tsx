...
interface ProfileContextType {
  ...
  updateProfile: (id: string, req: UpdateProfileRequest) => Promise<ServiceProfile>;
  ...
  setProfileEnvVar: (id: string, name: string, value: string, description?: string, isRequired?: boolean) => Promise<void>;
  deleteProfileEnvVar: (id: string, name: string) => Promise<void>;
  removeServiceFromProfile: (profileId: string, serviceName: string) => Promise<void>;
...
export function ProfileProvider({ children }: ProfileProviderProps) {
  ...
  const setActiveProfileFromProfiles = (profiles: ServiceProfile[]) => {
    console.log('Setting active profile from profiles:', profiles.map((p: ServiceProfile) => ({ name: p.name, isActive: p.isActive })));
    ...
    console.log('No active profile found in backend data, clearing active profile state');
  ...
  const fetchServiceProfiles = async () => {
    try {
      ...
      console.log('Fetched profiles:', profiles?.map((p: ServiceProfile) => ({ name: p.name, isActive: p.isActive })));
  ...
  const createProfile = async (req: CreateProfileRequest): Promise<ServiceProfile> => {
    try {
      setIsCreating(true);
  ...
  const updateProfile = async (id: string, req: UpdateProfileRequest): Promise<ServiceProfile> => {
    try {
      setIsUpdating(true);
  ...
  const getProfileEnvVars = async (id: string): Promise<Record<string, string>> => {
    try {
  ...
  const deleteProfileEnvVar = async (id: string, name: string) => {
    try {
      await apiCall(`/api/profiles/${id}/env-vars/${encodeURIComponent(name)}`, {
        method: 'DELETE',
      });
    } catch (error) {
  ...
  const removeServiceFromProfile = async (profileId: string, serviceName: string) => {
    try {
      await apiCall(`/api/profiles/${profileId}/services/${encodeURIComponent(serviceName)}`, {
        method: 'DELETE',
      });
    } catch (error) {
